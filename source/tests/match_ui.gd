...
func run() -> void:
	...
	root.add_child(game)
	await process_frame
	check(game.sound.nature.playing,"nature starts on home when enabled")
	game.sound._notification(Node.NOTIFICATION_APPLICATION_PAUSED)
	check(game.sound.nature.stream_paused,"backgrounding pauses nature")
	game.sound._notification(Node.NOTIFICATION_APPLICATION_RESUMED)
	check(not game.sound.nature.stream_paused,"foreground restores nature")
	await capture("home")
